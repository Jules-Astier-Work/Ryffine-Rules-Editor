...
class JingError:
	...
	def parse_error(self, error_str, content):
		...
		if error_match is not None:
			...
			# pattern = r'element "([^"]+)" not allowed anywhere(?:; expected element (.+))?'
			...
			# Pattern 1: Element not allowed
			pattern = r'element "([^"]+)" not allowed (?:yet|here|anywhere)(?:; expected element (.+))?'
			match = re.match(pattern, self.error)
			if match:
	...
	def get_error_text(self):
		if isinstance(self.error_rules, str):
			...
			return [f"DEGUB: ({self.error})"] + self.error_rules['rule'].tolist()
